...
def counter(name: str, description: str = "", unit: str = "") -> Counter:
    ...
    if meter is not None:
        try:
            return Counter(name, meter.create_counter(name=name, description=description, unit=unit))
        except Exception:
            return Counter(name)
    return Counter(name)
...
def gauge(name: str, description: str = "", unit: str = "") -> Gauge:
    ...
    if meter is not None:
        try:
            return Gauge(name, meter.create_up_down_counter(name=name, description=description, unit=unit))
        except Exception:
            return Gauge(name)
    return Gauge(name)
...
def histogram(name: str, description: str = "", unit: str = "") -> Histogram:
    ...
    if meter is not None:
        try:
            return Histogram(name, meter.create_histogram(name=name, description=description, unit=unit))
        except Exception:
